...
Source0:        https://github.com/JohnCremona/eclib/archive/v%{version}/%{name}-%{version}.tar.gz
...
sed -i 's/^LIBS.*/& -lpthread/' tests/Makefile.am
sed -i 's/^LIBS.*/& -lpthread/' progs/Makefile.am

# FLINT_LEVEL 2 assumes that the C int type == half the width of a limb_t.
# This is only true on 64 bit platforms.
if [ %{__isa_bits} = "64" ]; then
  sed -i '/FLINT_LEVEL/s/1/2/' configure.ac
fi
