...
cp %{buildroot}%{_datadir}/pixmaps/TeXmacs.xpm %{buildroot}%{_datadir}/icons/gnome/48x48/mimetypes/gnome-mime-text-texmacs.xpm
...
for i in `ls *.pfb`; do
        ...
        ln -s %{_fontdir}/$i %{buildroot}%{_datadir}/TeXmacs/fonts/type1/ec/$i
done
cd ../la
for i in `ls *.pfb`; do
        mv $i %{buildroot}%{_fontdir}
        ln -s %{_fontdir}/$i %{buildroot}%{_datadir}/TeXmacs/fonts/type1/la/$i
done
cd ../math
for i in `ls *.pfb`; do
        mv $i %{buildroot}%{_fontdir}
        ln -s %{_fontdir}/$i %{buildroot}%{_datadir}/TeXmacs/fonts/type1/math/$i
done
popd
...
%{_datadir}/icons/gnome/scalable/mimetypes/text-x-texmacs.svg
