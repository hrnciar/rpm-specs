...
cp -a  %{name}%{version} %{name}-%{version}-svga
make -C %{name}-%{version}-svga -f Makefile.svgalib %{makedirs} \
        %{_smp_mflags} DEFINES="%{optflags}"
%endif
...
make -C %{name}-%{version}-svga -f Makefile.svgalib install INSTALLSOUND=False \
