...
%prep
%autosetup -n %{module}-releases-%{version} -N
...
sed -i 's,\(@includedir@/coin\)-or,\1,' ipopt.pc.in
...
CXXLIBS="-L%{_libdir} -ldmumps $LIBBLAS $LIBLAPACK -lmetis -ldl"
%configure CC=gcc CXX=g++ F77=gfortran CFLAGS="$CFLAGS" CXXFLAGS="$CXXFLAGS" \
           LDFLAGS="$LDFLAGS" CXXLIBS="$CXXLIBS" \
 --with-mumps-cflags=-I%{_includedir}/MUMPS --with-mumps-lflags=-ldmumps \
 --with-lapack-cflags=$INCLAPACK --with-lapack-lflags=$LIBLAPACK \
%if 0%{?with_asl}
 --with-asl-cflags=-I%{_includedir}/asl --with-asl-lflags="-lasl -lmp" \
%endif
 --enable-shared

sed -i '/LATEX_BATCHMOD/s/NO/YES/' doc/Doxyfile
...
%configure --with-mumps-cflags="-I%{_includedir}/MUMPS $MPI_INCLUDE" \
 --with-mumps-lflags="-L$MPI_LIB -ldmumps -lmumps_common" \
 --with-lapack-cflags=$INCLAPACK --with-lapack-lflags=$LIBLAPACK \
%if 0%{?with_asl}
 --with-asl-cflags=-I%{_includedir}/asl --with-asl-lflags="-lasl -lmp" \
%endif
...
cd %{name}-%{version}-openmpi
 cp -p --no-dereference src/Interfaces/.libs/libipopt.so* \
...
install -p -m 644 %{name}-%{version}-openmpi/headers/* $RPM_BUILD_ROOT$MPI_INCLUDE
%{_openmpi_unload}
%endif
...
##Copy MPI header files
install -p -m 644 %{name}-%{version}-mpich/headers/* $RPM_BUILD_ROOT$MPI_INCLUDE
%{_mpich_unload}
%endif
...
cd %{name}-%{version}-serial
 ...
 cp -p --no-dereference contrib/sIPOPT/src/.libs/libsipopt.so* \
       $RPM_BUILD_ROOT%{_libdir}/
 cp -p --no-dereference ipopt.pc $RPM_BUILD_ROOT%{_libdir}/pkgconfig
%if 0%{?with_asl}
 cp -p --no-dereference src/Apps/AmplSolver/.libs/libipoptamplinterface.so* \
       $RPM_BUILD_ROOT%{_libdir}/
 cp -p --no-dereference ipoptamplinterface.pc \
...
# Disable mpich tests until bz 1803964 is fixed
%if 0
%if 0%{?with_mpich}
%{_mpich_load}
cd %{name}-%{version}-mpich
