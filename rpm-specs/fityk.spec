...
Source0:        https://github.com/wojdyr/%{name}/archive/v%{version}/%{name}-%{version}.tar.gz
# Remove next release
Source1:        https://raw.githubusercontent.com/wojdyr/%{name}/master/%{name}.appdata.xml
...
cp -p %{SOURCE1} %{buildroot}%{_metainfodir}/%{name}.appdata.xml
