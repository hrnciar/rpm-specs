...
%global  commit      6e868bcc692fc46819ac56e1b1fae2d7b272edca
...
Obsoletes: preprocess < 0:1.2.3-0.10
Provides:  preprocess = 0:%{version}-%{release}
%endif
%if 0%{?rhel}
...
%if 0%{?with_python2}
cp -a preprocess-%{commit} python2
pushd python2
...
%if 0%{?with_python3}
cp -a preprocess-%{commit} python3
find python3 -name '*.py' | xargs sed -i '1s|^#!/usr/bin/env python|#!%{__python3}|'
%endif
# with_python3

%if 0%{?with_python3_other}
cp -a preprocess-%{commit} python%{python3_other_pkgversion}
