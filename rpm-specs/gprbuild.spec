...
Source0:    gprbuild-gpl-2018-20180524-src.tar.gz 
...
%ifnarch %{bootstrap_arch}
%package doc
Summary:        Documentaion for GPRbuild
License:        GPLv3+
...
%if %{with_libgpr}
%package -n libgpr
Summary:        Ada library to handle GPRbuild project files
License:        GPLv3+
...
Summary:        Devel package for %{name}-libgpr
License:        GPLv3+
...
XMLADA_BUILD=shared make ADA_PROJECT_PATH=%_GNAT_project_dir BUILDER="gprbuild -v %{GPRbuild_optflags}" all BUILD=debug
%if %{with_libgpr}
XMLADA_BUILD=shared make ADA_PROJECT_PATH=%_GNAT_project_dir BUILDER="gprbuild -v %{GPRbuild_optflags}" libgpr.build.shared BUILD=debug
%endif
%endif
...
%dir %{_libdir}/gpr
%{_libdir}/libgpr.so
%{_libdir}/gpr/relocatable/gpr/libgpr.so
