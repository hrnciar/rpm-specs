...
sed -e 's|-fvisibility=hidden||g' -i configure gecode.m4
...
%if 0%{?rhel} && 0%{?rhel} < 7
export CXXFLAGS="%{optflags} -Wl,-z,relro -fPIC -pie -Wl,-z,now -std=gnu++0x"
export CPPFLAGS=" -std=gnu++0x"
export LDFLAGS="%{__global_ldflags} -fPIC -pie -Wl,-z,now -lpthread"
%else
