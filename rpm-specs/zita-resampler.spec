...
%build
export LDFLAGS="-L../source $RPM_LD_FLAGS"
...
%install
make DESTDIR=%{buildroot} PREFIX=%{_prefix} LIBDIR=%{_libdir} -C source install
make DESTDIR=%{buildroot} MANDIR=%{_mandir}/man1 PREFIX=%{_prefix} LIBDIR=%{_libdir} -C apps install
