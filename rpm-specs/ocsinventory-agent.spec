...
Source30:   ocsinventory-agent.service
Source31:   ocsinventory-agent-onboot.timer
Source32:   ocsinventory-agent-hourly.timer
Source33:   ocsinventory-agent-daily.timer

Patch0001:	fix_unused_user.diff
Patch0002:	fix_syntax.diff
Patch0003:	fix_lspci.diff
Patch0004:	ca-certs.diff
Patch0005:	fix_utf8.diff
...
sed -e 's;/etc/;%{_sysconfdir}/;' \
    ...
    %{SOURCE2} > %{buildroot}%{_libexecdir}/%{name}/ocsinventory-agent.cron

mkdir -p %{buildroot}/%{_unitdir}
install -pm 644 %{SOURCE30}   %{buildroot}/%{_unitdir}/
install -pm 644 %{SOURCE31}   %{buildroot}/%{_unitdir}/
install -pm 644 %{SOURCE32}   %{buildroot}/%{_unitdir}/
install -pm 644 %{SOURCE33}   %{buildroot}/%{_unitdir}/
