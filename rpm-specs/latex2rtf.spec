...
# Fix build for perl-5.26
Patch2:         latex2rtf-perl526.patch
BuildRequires:  gcc
...
%make_build %{name} CFLAGS="%{optflags} -DUNIX -fsigned-char" LDFLAGS="%{?__global_ldflags}" DESTDIR=%{_prefix} CFGDIR=/share/%{name}/cfg

%install
make install DESTDIR=%{buildroot}%{_prefix}
make install-info DESTDIR=%{buildroot}%{_prefix}
