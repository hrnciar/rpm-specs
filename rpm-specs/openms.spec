...
BuildRequires: seqan-devel <= 0:1.4.2
...
Requires: fido-pi%{?_isa}
Requires: percolator%{?_isa}
Requires: R-core%{?_isa}
Requires: %{name}-data%{?_isa} = %{version}-%{release}

# Python binding looks need too much memory to be compiled
%global with_pyOpenMS 0
Obsoletes: python3-openms < 0:2.5.0-1
...
%else
%make_build OpenMS TOPP UTILS GUI
%endif

%if 0%{?with_pyOpenMS}
export LD_LIBRARY_PATH=$PWD/lib:$LD_LIBRARY_PATH
make --jobs 4 pyopenms
%endif
popd

%install
export LD_LIBRARY_PATH=$PWD/lib:$LD_LIBRARY_PATH
xvfb-run -a %make_install -C build
...
desktop-file-install \
 --set-key=Exec --set-value="env LD_LIBRARY_PATH=%{_libdir}/OpenMS OPENMS_DATA_PATH=%{_datadir}/OpenMS TOPPAS" \
 --set-icon=%{_datadir}/icons/TOPP/TOPPAS.png \
 --set-key=StartupNotify --set-value=true \
 --add-category=Utility --add-category=Development \
...
desktop-file-install \
 --add-category=Utility --add-category=Development \
 --set-key=Exec --set-value="env LD_LIBRARY_PATH=%{_libdir}/OpenMS OPENMS_DATA_PATH=%{_datadir}/OpenMS INIFileEditor" \
...
%license LICENSE
%{_datadir}/OpenMS/
%exclude %{_datadir}/OpenMS/examples/external_code/*.h
%exclude %{_datadir}/OpenMS/examples/external_code/*.cpp
...
%doc CHANGELOG AUTHORS README
%{_includedir}/OpenMS/
%{_datadir}/OpenMS/examples/external_code/*.h
%{_datadir}/OpenMS/examples/external_code/*.cpp
