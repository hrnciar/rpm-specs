...
Source22:  munin-node.rc
Source23:  munin-node.service
Source24:  munin-asyncd.rc
Source25:  munin-asyncd.service
Source26:  munin-node.xml

# CGI sources
Source30:  munin-cgi-html.rc
Source31:  munin-cgi-graph.rc
...
%endif
BuildArch: noarch
Provides:  munin-async = %{version}-%{release}
Obsoletes: munin-async < 2.0.39-4
Provides:  munin-netip-plugins = %{version}-%{release}
Obsoletes: munin-netip-plugins < 2.0.39-4
Provides:  munin-java-plugins = %{version}-%{release}
Obsoletes: munin-java-plugins < 2.0.39-2
Provides:  munin-plugins-java = %{version}-%{release}
Obsoletes: munin-plugins-java < 2.0.45-2
...
BuildArch: noarch
Provides:  munin-ruby-plugins = %{version}-%{release}
Obsoletes: munin-ruby-plugins < 2.0.39-2
...
Requires:  nginx-filesystem
BuildArch: noarch
Provides:  munin-fcgi = %{version}-%{release}
Obsoletes: munin-fcgi < 2.0.39-4
%if 0%{?rhel} >= 7 || 0%{?fedora} >= 16
%{?systemd_requires}
%else
Requires(post):  /sbin/chkconfig
...
cp %{SOURCE22} %{buildroot}%{_initrddir}/munin-node
cp %{SOURCE24} %{buildroot}%{_initrddir}/munin-asyncd
%endif
...
# Add server cron job
%if 0%{?rhel} >= 7 || 0%{?fedora} >= 16
cp %{SOURCE131} %{buildroot}%{_unitdir}
cp %{SOURCE132} %{buildroot}%{_unitdir}
%else
...
%else
mkdir -p %{buildroot}%{_initrddir}
cp %{SOURCE30} %{buildroot}%{_initrddir}/munin-cgi-html
cp %{SOURCE31} %{buildroot}%{_initrddir}/munin-cgi-graph
%endif
...
useradd -r -g munin -d %{_sharedstatedir}/munin -s /bin/sh -c "Munin user" munin
exit 0
...
if [ -f %{_sysconfdir}/cron.d/munin ]; then
    systemctl enable --now munin.timer > /dev/null 2>&1 || :
fi
...
%triggerun -- munin < 2.0.42-2
setfacl --remove-all %{_rundir}/munin
chmod 775 %{_rundir}/munin
%endif
...
[ -f %{_localstatedir}/log/munin-node/munin-node.log ] || \
    /usr/bin/install -m 0640 -o root -g root /dev/null %{_localstatedir}/log/munin-node/munin-node.log
...
if [ "$1" = "1" ]; then
    ...
    if [ -x %{_bindir}/ipcs -a -x %{_bindir}/ipcrm ]; then
        for mem in $(%{_bindir}/ipcs --shmems | grep munin | cut --delimiter=' ' --fields=1)
        do
            %{_bindir}/ipcrm --shmem-key ${mem} 2>/dev/null || :
        done
        for sem in $(%{_bindir}/ipcs --semaphores | grep munin | cut --delimiter=' ' --fields=1)
        do
...
if [ $1 -eq 0 ]; then
    ...
    /sbin/service munin-asyncd stop >/dev/null 2>&1 || :
...
%else
if [ $1 -ge 1 ]; then
    /sbin/service munin-node condrestart >/dev/null 2>&1
    /sbin/service munin-asyncd condrestart >/dev/null 2>&1
fi
%endif


%triggerun node -- munin-node < 2.0.40-2
if ls %{_localstatedir}/log/munin/munin-node.log* >/dev/null 2>&1; then
    mv -f %{_localstatedir}/log/munin/munin-node.log* %{_localstatedir}/log/munin-node/
fi
chown -R munin:munin %{_sharedstatedir}/munin/spool/ >/dev/null 2>&1
%if 0%{?rhel} >= 7 || 0%{?fedora} >= 16
...
%if 0%{?rhel} >= 7 || 0%{?fedora} >= 16
%triggerun common -- munin-common < 2.0.42-2
setfacl --remove-all %{_rundir}/munin
chmod 775 %{_rundir}/munin
%endif
...
%else
/sbin/chkconfig --add munin-cgi-html
/sbin/chkconfig --add munin-cgi-graph
%endif
...
%else
if [ $1 -eq 0 ]; then
    /sbin/service munin-cgi-html stop >/dev/null 2>&1 || :
    /sbin/chkconfig --del munin-cgi-html
    /sbin/service munin-cgi-graph stop >/dev/null 2>&1 || :
    /sbin/chkconfig --del munin-cgi-graph
fi
%endif
...
%else
if [ $1 -ge 1 ]; then
    /sbin/service munin-cgi-html condrestart >/dev/null 2>&1
    /sbin/service munin-cgi-graph condrestart >/dev/null 2>&1
fi
%endif
...
%endif
%attr(-, munin, munin) %dir %{_sharedstatedir}/munin
%attr(0775, root, munin) %dir %{_sharedstatedir}/munin/plugin-state
%attr(-, root, munin) %dir %{_sharedstatedir}/munin/plugin-state/root
%attr(-, munin, munin) %dir %{_sharedstatedir}/munin/plugin-state/munin
%attr(-, munin, munin) %dir %{_sharedstatedir}/munin/spool
%attr(0700, munin, munin) %dir %{_sharedstatedir}/munin/.ssh
%attr(0755, root, root) %dir %{_localstatedir}/log/munin-node
%attr(0640, root, root) %ghost %{_localstatedir}/log/munin-node/munin-node.log
...
%else
%attr(0775, root, munin) %dir %{_localstatedir}/run/munin
%endif
...
%else
%{_initrddir}/munin-cgi-html
%{_initrddir}/munin-cgi-graph
%endif
