...
# sys_errlist() looks deprecated now
Patch0:  %{name}-0.5.3-FTBFS-sys-errlist.patch

%if 0%{?fedora} || 0%{?rhel} > 7
Patch1:   Fix-the-cipher-list-string-to-ensure-that-it-contain.patch
%endif
...
# Fix compilation error "too few arguments to function 'longjmp'" in F26
sed -i "s/longjmp(env)/longjmp(env, val)/g" config.h.in
