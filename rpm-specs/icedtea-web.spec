...
Source0:    https://github.com/AdoptOpenJDK/IcedTea-Web/archive/%{name}-%{version}-pre.0.alpha13.patched2.tar.xz.tar.xz
...
gconftool-2 -s  %{jnlphandler}/command  '%{_libexecdir}/%{name}/javaws %s' --type String &> /dev/null || :
gconftool-2 -s  %{jnlphandler}/enabled  --type Boolean true &> /dev/null || :
gconftool-2 -s %{jnlpshandler}/command '%{_libexecdir}/%{name}/javaws %s' --type String &> /dev/null || :
