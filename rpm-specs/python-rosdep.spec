...
BuildRequires:  python2-rospkg >= 1.1.10
...
Requires:       python2-rospkg >= 1.1.10
%endif
...
BuildRequires:  python%{python3_pkgversion}-rospkg >= 1.1.10
Requires:       python-srpm-macros
...
Requires:       python%{python3_pkgversion}-rospkg >= 1.1.10
%endif
...
# ros-infrastructure/rosdep#755
sed -i '1{s|^#!/usr/bin/\(env \)\?python$||}' src/rosdep2/install.py
...
%check
%if 0%{?with_python2}
PYTHONPATH=%{buildroot}%{python2_sitelib} %{__python2} -m nose test.test_rosdep_redhat
%endif

%if 0%{?with_python3}
PYTHONPATH=%{buildroot}%{python3_sitelib} %{__python3} -m nose test.test_rosdep_redhat
%endif
