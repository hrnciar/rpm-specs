...
Source0: http://download.wikimedia.org/mediawiki/1.34/%{name}-%{version}.tar.gz
Source1: http://download.wikimedia.org/mediawiki/1.34/%{name}-%{version}.tar.gz.sig
...
BuildRequires: php-composer(cssjanus/cssjanus) >= 1.3.0
...
BuildRequires: php-composer(wikimedia/ip-set) >= 2.1.0
BuildRequires: php-composer(wikimedia/utfnormal) >= 2.0.0
BuildRequires: php-composer(zordius/lightncandy) >= 0.23
BuildRequires: php-pear(Mail) >= 1.4.1
BuildRequires: php-pear(Mail_Mime) >= 1.10.2
...
Requires: php-composer(cssjanus/cssjanus) >= 1.3.0
...
Requires: php-composer(wikimedia/ip-set) >= 2.1.0
Requires: php-composer(wikimedia/utfnormal) >= 2.0.0
Requires: php-composer(zordius/lightncandy) >= 0.23
Requires: php-pear(Mail) >= 1.4.1
Requires: php-pear(Mail_Mime) >= 1.10.2
...
Provides: bundled(php-christian-riesen-base32) = 1.3.2
Provides: bundled(php-guzzlehttp-guzzle) = 6.3.3
Provides: bundled(php-guzzlehttp-promises) = 1.3.1
Provides: bundled(php-guzzlehttp-psr7) = 1.5.2
Provides: bundled(php-james-heinrich-getid3) = 1.9.14
Provides: bundled(php-jakobo-hotp-php) = 1.0.1
...
Provides: bundled(php-wikimedia-at-ease) = 1.2.0
Provides: bundled(php-wikimedia-remex-html) = 1.0.1
Provides: bundled(php-wikimedia-timestamp) = 1.0.0
Provides: bundled(php-wikimedia-base-convert) = 2.0.0
Provides: bundled(php-wikimedia-cldr-plural-rule-parser) = 1.0.0
Provides: bundled(php-wikimedia-composer-merge-plugin) = 1.4.1
Provides: bundled(php-wikimedia-html-formatter) = 1.0.2
Provides: bundled(php-wikimedia-less.php) = 1.8.0
Provides: bundled(php-wikimedia-object-factory) = 1.0.0
Provides: bundled(php-wikimedia-password-blacklist) = 0.1.4
Provides: bundled(php-wikimedia-php-session-serializer) = 1.0.7
Provides: bundled(php-wikimedia-purtle) = 1.0.7
Provides: bundled(php-wikimedia-relpath) = 2.1.1
Provides: bundled(php-wikimedia-remex-html) = 2.0.1
Provides: bundled(php-wikimedia-running-stat) = 1.2.1
Provides: bundled(php-wikimedia-scoped-callback) = 3.0.0
Provides: bundled(php-wikimedia-timestamp) = 2.2.0
Provides: bundled(php-wikimedia-wait-condition-loop) = 1.0.1
Provides: bundled(php-wikimedia-wrappedstring) = 3.0.1
Provides: bundled(php-wikimedia-xmp-reader) = 0.6.2
...
echo "require dirname(dirname(__FILE__)) . '/vendor/wikimedia/relpath/src/RelPath/RelPath.php';" >> vendor/autoload.php
...
%files
%doc FAQ HISTORY README README.RPM RELEASE-NOTES-1.34 UPGRADE CREDITS docs
%license COPYING
%{_datadir}/mediawiki
/var/www/wiki
